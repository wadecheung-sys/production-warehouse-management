...
function renderCharts() {
  ...
  if (ageChartRef.value) {
    ...
    ageChart.setOption({
      tooltip: {},
      xAxis: { type: 'category', data: ageData.value.map((d) => d.name) },
      yAxis: { type: 'value' },
      series: [{ type: 'bar', data: ageData.value.map((d) => d.value), itemStyle: { color: '#1677ff' } }],
...
function renderMap() {
  ...
  for (const c of cityBubbles.value) {
    ...
    marker.bindPopup(
      `<b>${c.name}</b><br/>备品 ${c.spare}<br/>仪器 ${c.instrument}<br/>工器具 ${c.tool}<br/><small>点击气泡进入台账</small>`,
    )
...
.analytics-home {
  .hero-bar {
    background: linear-gradient(120deg, #e8f3ff 0%, #f5f9ff 45%, #fff 100%);
